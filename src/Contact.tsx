...
import { databases, account, DATABASE_ID, REQUEST_COLLECTION_ID } from './appwrite';

function Contact() {
  const [formData, setFormData] = useState({ name: '', phone: '', email: '', message: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  // 🔴 ম্যাজিক: কাস্টমার লগইন করা থাকলে তার নাম ও ইমেইল অটো-ফিল করে দেবে!
  useEffect(() => {
    const fetchLoggedInUser = async () => {
      try {
        const user = await account.get();
        // ইউজারের নাম আর ইমেইল ফর্মের স্টেটে বসিয়ে দেওয়া হচ্ছে
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData(prev => ({ ...prev, phone: '', message: '' })); // নাম ও ইমেইল রেখে বাকিটা ক্লিয়ার
    } catch (error) {
      ...
      <main className="max-w-[1000px] mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-slate-100">
          <div className="space-y-8 bg-slate-50 p-8 rounded-2xl border border-slate-100 h-fit">
            <div><h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-2">Office Address</h3><p className="text-slate-800 font-bold text-lg">Dhaka, Bangladesh</p></div>
            <div><h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-2">Email Us</h3><p className="text-orange-600 font-bold text-lg">support@bdtplatform.com</p></div>
            <div><h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-2">Call Us</h3><p className="text-slate-800 font-bold text-lg">+880 1XXX-XXXXXX</p></div>
          </div>
